structure blibNumInt :> blibNumInt =
struct
...
fun NUMCONV_ERR function message =
  HOL_ERR{origin_structure = "blibNumInt",
...
fun INTF_TAC goal =
  ...
  let val finalgoal = (erase_aconv (defl @ randl),F) in
...
fun intbv_conv_FST term =   
  let val (bv,t) = dest_forall term in
    ...
    then
      ...
      let val def = mk_eq (mk_var ("P",``:int -> bool``),t2) in
      let val eqth4 = ASSUME def in
      let val th5 = SUBS [eqth4] th3 in
      let val th7 = CONV_RULE (REDEPTH_CONV BETA_CONV) th5 in
        REMOVE_DEFL [def] th7
      end end end end end 
      end end end end end 
