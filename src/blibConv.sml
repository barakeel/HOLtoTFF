...
fun inj_fun_def term usedv =
  ...
  let val t1 = list_mk_comb (op1,argl1) in
  let val argtyl2 = map type_num_to_int argtyl1 in
  let val opty2 = mk_funtype argtyl2 (type_of term) in
...
fun inj_fun_def2 term usedv =
  let val (_,arg) = dest_comb term in
  let val (op1,argl) = strip_comb arg in
  let val argtyl1 = map type_of argl in 
  let val namel = 
    create_newnamel_aux "x" (length argl) (map name_of usedv) 
  in  
  let val argl1 = list_mk_var (namel,argtyl1) in
  let val t1 = mk_comb (int_injection ,list_mk_comb (op1,argl1)) in
  let val argtyl2 = map type_num_to_int argtyl1 in
...
val term = ``P  ((\z.z) x) ((\z.z) y)``;
*)
 

 
(* look at the case where the operator is & - +  etc *)
(* test
val term = ``x + y = z:num``;
val term = ``x + y < z:num``;
val term = ``x + y <= z:num``;

INT_NUM_FUN_CONV_SUB (all_var term) term
val t = ``x + 2 * y:num``;
val t = ``x - 2 * y:num``;
*)
...
fun INT_NUM_FUN_CONV_SUB usedv term =
  ...
  then
    if not (is_comb term) then raise UNCHANGED      
    else
      ...
      | _ => let val (_,argl1) = strip_comb t in 
             let val def = inj_fun_def2 term usedv in
               SPECL argl1 (ASSUME def)
             end end
      end
  else
    if not (is_comb term) then raise UNCHANGED
...
val term = ``f (z:num) h = (0:num)``;
val term = ``!z:num. f z h = (0:num)``;
val term = ``P (x:num) (y:bool):bool``;
val usedv = ;
show_assums := true;
val thm = INT_NUM_FUN_CONV (all_var term) term;

is_const (mk_var ("=",``:int -> int -> bool``));
...
fun INT_NUM_FORALL_CONV_ALL term =
  if is_forall term 
  then 
    ((QUANT_CONV INT_NUM_FORALL_CONV_ALL) THENC INT_NUM_FORALL_CONV_FST)
      term  
  else 
    raise UNCHANGED 
...
fun numfun_axiom revextdef =
  ...
  let val (oper,arg) = dest_comb t1 in
    ...
    then
      ...
      let val th5 = (INT_NUM_FORALL_CONV_ALL THENC normalForms.CNF_CONV)
                     (concl th4) 
      in
...
(* val revextdef = ``∀x x1. (& (f x x1)) = (f' (&x) x1 : int)``; *)
 ...
 (* test
 val term = ``!a:num c:bool b:num d:num. f (&b:int) (c:bool) (g (&d:int))``;
 val term = ``!b:num. f (&a:int) (&b:int)``;
 val thm = (INT_NUM_FORALL_CONV_ALL THENC normalForms.CNF_CONV) term;
 EQ_MP thm (ASSUME term);
 remove_unused_def (hd (hyp thm)) thm;
  val numargl = [``a:num``];
 val usedvarl = [];
 val newoperator = ``f:num -> bool -> bool``;
show_assums:= true;
 *)
(* 
load "blibExtractvar";
